...
func TestUpdateEntry(t *testing.T) {
	...
	updatedEntry, exists := hdbt.hdb.hostTrees.Select(entry1.PublicKey)
	if !exists {
	...
	updatedEntry, exists = hdbt.hdb.hostTrees.Select(entry2.PublicKey)
	if !exists {
		t.Fatal("Entry did not get inserted into the host tree")
	}
	if len(updatedEntry.ScanHistory) != 2 {
	...
	err = hdbt.hdb.hostTrees.Modify(updatedEntry)
	if err != nil {
		t.Fatal(err)
	}
